...
@section('content')
    <div class="max-w-3xl mx-auto px-4 py-8">
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <form action="{{ route('customer.post.update', $posts->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-8 p-6">
                    <div>
                        ...
                        <textarea name="content" id="content" rows="6"
                            ...
                            placeholder="Share your thoughts with your audience..." required>{{ old('content', $posts->content) }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            ...
                            <div class="relative">
                                <div
                                    class="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-gray-400 transition-colors duration-200">
                                    <div class="space-y-2 text-center">
                                        ...
                                        <div class="text-sm text-gray-600">
                                            <label for="image"
                                                ...
                                                <input id="image" name="image[]" type="file" accept="image/*" class="sr-only" multiple>
                                            </label>
                                        </div>
                                ...
                                @if ($posts->image)
                                    <div id="existing-images" class="mt-4 flex flex-wrap gap-4">
                                        @foreach (json_decode($posts->image) as $image)
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <label for="visibility"
                                    class="block text-sm font-medium text-gray-700 mb-2">Visibility</label>
                                <select name="visibility" id="visibility"
                                    class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-200">
                                    <option value="pharmacy" {{ $posts->visibility == 'pharmacy' ? 'selected' : '' }}>
                                        Pharmacy Only</option>
                                    <option value="public" {{ $posts->visibility == 'public' ? 'selected' : '' }}>Public
                                    </option>
                                </select>
                            </div>
                        </div>
    ...
    <script>
        function removeExistingImage(button, imagePath) {
            ...
            document.getElementById('removed-images-container').appendChild(input);
        }
    </script>
