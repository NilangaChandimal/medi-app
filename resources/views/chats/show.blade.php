        ...
        <div class="flex-1 flex flex-col bg-gray-50">
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-4" id="chat-messages">
                @foreach($messages as $message)
                    <div class="flex {{ $message->sender_id == Auth::id() && get_class(Auth::user()) == $message->sender_type ? 'justify-end' : 'justify-start' }}">
                        <div class="message-bubble max-w-[70%] rounded-2xl px-4 py-3
                            ...
                            @if(isset($message->file_path))
                                <img src="{{ asset('storage/' . $message->file_path) }}"
                                     class="mt-2 rounded-lg max-w-xs cursor-pointer clickable-image
                ...
                <form method="POST" action="{{ route($userType . '.chats.storeMessage', $chat->id) }}"
                    ...
                    <div class="chat-input flex items-center gap-2 rounded-full p-2">
                        <button type="button" id="emoji-button" class="chat-button p-2 hover:bg-gray-100 rounded-full">
                            <svg class="h-6 w-6 text-indigo-600" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <button type="submit" class="chat-button p-2 bg-indigo-600 hover:bg-indigo-700 rounded-full transition-colors">
